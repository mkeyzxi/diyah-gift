...
const SalempangLayout = () => {
    ...
    return (
        <div 
            className="w-full h-12 bg-[var(--yellow)] overflow-x-hidden border-t-5 border-[var(--blackCustom)] shadow-salempang"
            role="region"
            aria-label="Animated Triangle Background"
        >
            <div className="flex animate-righttoleft border-[var(--blackCustom)]" aria-live="polite">
                {Array.from({ length: triangleCount }, (_, i) => (
                    <Triangle 
                        key={i} 
                        addClass="hover:scale-115 transition-all"
                        aria-hidden="true" 
                    />
                ))}
            </div>
