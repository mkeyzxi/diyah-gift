		...
		>
			<div className="flex justify-between items-center p-2">
				...
				<button
					...
					aria-pressed={thisLike}
					aria-label={thisLike ? "Batalkan suka" : "Sukai produk ini"}
				>
					{thisLike ? '♥ Liked' : '♡ Like'}
				</button>
			</div>
			<div
				className="space-y-3 p-2 overflow-y-auto max-h-[300px]"
				role="list"
			...
			>
				{items.map((item, index) => (
					<div
						...
						className="flex items-center bg-white rounded-lg shadow p-2"
						role="listitem"
						aria-label={`Produk ${item.name} seharga ${item.price}`}
					>
						<img
							src={item.image}
